...
DepGraph.prototype = {
  ...
  removeNode: function (node) {
    if (this.hasNode(node)) {
      ...
      var outgoingEdges = this.outgoingEdges[node];
      var incomingEdges = this.incomingEdges[node];
      delete this.outgoingEdges[node];
      delete this.incomingEdges[node];

      outgoingEdges.forEach(function (edgeNode) {
        var idx = this.incomingEdges[edgeNode].indexOf(node);
        if (idx >= 0) {
          this.incomingEdges[edgeNode].splice(idx, 1);
        }
      }, this);
      incomingEdges.forEach(function (edgeNode) {
        var idx = this.outgoingEdges[edgeNode].indexOf(node);
        if (idx >= 0) {
          this.outgoingEdges[edgeNode].splice(idx, 1);
        }
      }, this);
    }
  },
